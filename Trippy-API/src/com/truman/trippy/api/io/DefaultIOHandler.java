  ...
  public Response fetchData(String url, Method method) {
   ...
   trustAllHosts();
    try {
      ...
      if (aUrl.getProtocol().toLowerCase().equals("https")) {
              HttpsURLConnection https = (HttpsURLConnection) aUrl.openConnection();
              https.setHostnameVerifier(DO_NOT_VERIFY);
              connection = https;
          	  manager.setCookies(connection);
      } else {
      	  manager.setCookies(connection);
...
//        connection.setDoOutput(true);
        ...
        if (code == 200) {
          manager.storeCookies(connection);
